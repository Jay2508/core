...
{
    public class ProductTblService : IProductTblService, IDisposable
    {
        private readonly EntityDbContext db;

        public ProductTblService(EntityDbContext db)
        {
            this.db = db;
        }

        public async Task<ApiHttpResponsecs> Add(ProductTbl Model)
        {
            try
            {
                if (Model == null)
                ...
                var Data = await db.ProductTbls.Where(m => m.Name == Model.Name).FirstOrDefaultAsync();
                if (Data != null)
                {
                    return ApiHttpResponsecs.CreatResponse(Massage.AllReadyExist, ResponseStatus.Conflict, 404);
                }
                db.ProductTbls.Add(Model);
        ...
        public async Task<ApiHttpResponsecs> DeleteByid(int id)
        {
            try
            {
                ...
                var Data = await db.ProductTbls.FindAsync(id);
                if (Data == null)
                {
        ...
        public async Task<ApiHttpResponsecs> List()
        {
            try
            {
                var Data = await (from Pt in db.ProductTbls
                                  join Ct in db.CategoryTbls on Pt.CatId equals Ct.CatId
                                  join SCt in db.SubCategoryTbls on Pt.SubCatId equals SCt.SubCatId
                                  join TCt in db.ThirdCategoryTbls on Pt.ThierdCatId equals TCt.ThierdCatId
                                  join Bd in db.BrandTbls on Pt.BrandId equals Bd.BrandId
                                  select new ProductTblDto
                                  {
                                      ProductId = Pt.ProductId,
                                      Name = Pt.Name,
                                      Price = Convert.ToDecimal(Pt.Price),
                                      Category = Ct.Category,
                                      SubCategory = SCt.SubCategory,
                                      ThirdCategory = TCt.ThirdCategory,
                                      Brand = Bd.Brand,
                                      ProPhoto = Pt.Photo,
        ...
        public async Task<ApiHttpResponsecs> Update(ProductTbl Model, int id)
        {
            try
            {
                if (Model == null)
                ...
                Data.ThierdCatId = Model.ThierdCatId;
    ...
    public interface IProductTblService
    {

        Task<ApiHttpResponsecs> Add(ProductTbl Model);
        Task<ApiHttpResponsecs> Update(ProductTbl Model, int id);
